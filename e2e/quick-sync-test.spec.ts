...
test('Classroom Whiteboard Sync - Teacher to Student', async ({ page }) => {
  test.setTimeout(90000); // 增加超時時間，因為有網路延遲模擬和多步驟準備流程
  ...
  // 設定視口大小
  await teacherPage.setViewportSize({ width: 1200, height: 800 });
  ...
  try {
    ...
    const setupNetworkDelay = async (page: Page) => {
      await page.route('**/*', async (route) => {
        await new Promise(r => setTimeout(r, 250)); // 往返 500ms
    ...
    if (box && canvasExists) {
      ...
      console.log('  • 開始繪製 20 個隨機線條進行壓力測試...');
      for (let j = 0; j < 20; j++) {
        ...
        for (let i = 1; i <= steps; i++) {
          ...
          await canvas.hover({ position: { x: curX, y: curY }, force: true });
        }
        
        await teacherPage.mouse.up();
        // 隨機等待 50-150ms 模擬真實頻率
        await new Promise(r => setTimeout(r, 50 + Math.random() * 100));
      }
      console.log('  ✓ 20 個線條繪製完成');
      ...
      for (let i = 0; i < 300; i++) {
        ...
        const studentState: any = await studentPage.evaluate(() => {
          try {
            ...
            return { 
              ...
              logs: (window as any).__whiteboard_logs?.slice(-3) || [],
              ready: (window as any).__classroom_ready,
        ...
        // 同時檢查筆劃數量與像素
        if (studentState.strokeCount >= targetStrokeCount && studentState.pixelCount > 100) {
          console.log(`  ✅ 完全同步成功! 延遲: ${(i + 1) * 100}ms, 筆劃數: ${studentState.strokeCount}, 像素數: ${studentState.pixelCount}`);
          found = true;
          break;
        }

        if ((i + 1) % 50 === 0) {
          const info = studentState.error ? `Error: ${studentState.error}` : 
            `Ready: ${studentState.ready} | Strokes: ${studentState.strokeCount}/${targetStrokeCount} | Pixels: ${studentState.pixelCount} | Canvases: ${studentState.canvasCount}`;
